...
Describe 'v3 manifest category modules — <Category>' -ForEach @(
    Get-ChildItem (Join-Path $script:RepoRoot 'manifests/collectors') -Directory | Sort-Object Name |
        ForEach-Object { @{ Category = $_.Name; Path = $_.FullName } }
) {
    It 'has at least one valid collector definition' {
        $Collectors = @(Get-ScoutCollector -DefinitionRoot (Join-Path $script:RepoRoot 'manifests/collectors') -Category $Category)
