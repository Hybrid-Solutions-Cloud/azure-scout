...
Describe 'AB#6890 — LandingZone scope is enumerated, not globbed' {
    ...
    It 'does not declare a bare waf.* glob' {
        ...
        @($script:Manifest.LandingZone.Rules) | Should -Not -Contain 'waf.*'
    }

    It 'names exactly the five WAF pillars' {
        $waf = @(@($script:Manifest.LandingZone.Rules) | Where-Object { $_ -like 'waf.*' } | Sort-Object)
        $waf | Should -Be @(
            'waf.cost'
            'waf.operational'
            'waf.performance'
    ...
    It 'excludes every caf rule file that belongs to another assessment' {
        ...
        $declared = @($script:Manifest.LandingZone.Rules)
        $others = foreach ($k in $script:Manifest.Keys) {
            if ($k -eq 'LandingZone') { continue }
    ...
    It 'does not declare a bare caf.* glob either' {
        ...
        @($script:Manifest.LandingZone.Rules) | Should -Not -Contain 'caf.*'
    }

    It 'names exactly the eight CAF landing-zone design areas' {
        $caf = @(@($script:Manifest.LandingZone.Rules) | Where-Object { $_ -like 'caf.*' } | Sort-Object)
        $caf | Should -Be @(
            'caf.billing'
            'caf.governance'
            'caf.identity'
    ...
    It 'still covers the cross-resource rules' {
        ...
        @($script:Manifest.LandingZone.Rules) | Should -Contain 'xr.*'
    }
    ...
    It 'every enumerated pillar file actually exists' {
        ...
        foreach ($p in @($script:Manifest.LandingZone.Rules | Where-Object { $_ -notmatch '\*' })) {
