...
[void]$sb.AppendLine('---')
[void]$sb.AppendLine('description: The held legacy worksheet metadata and columns declared by each Azure Scout collector.')
[void]$sb.AppendLine('---')
[void]$sb.AppendLine()
[void]$sb.AppendLine('# Collector Fields')
[void]$sb.AppendLine()
if ($WithColumns -eq $Total) {
    [void]$sb.AppendLine("What each collector can shape internally. **All $Total collectors** declare legacy worksheet")
    [void]$sb.AppendLine('metadata and an ordered column list.')
}
else {
    [void]$sb.AppendLine("What each collector can shape internally. **$WithColumns of $Total collectors** declare legacy worksheet metadata")
    [void]$sb.AppendLine('and an ordered column list; the rest feed the assessment engine or another collector rather')
    [void]$sb.AppendLine('than declaring a worksheet shape of their own.')
}
[void]$sb.AppendLine()
[void]$sb.AppendLine('[ARM Modules](./arm-modules.md) answers *what is covered* — collector to resource type.')
[void]$sb.AppendLine('This page answers which fields the processing layer can shape for JSON and React, and preserves')
[void]$sb.AppendLine('the worksheet names used by the held legacy Excel implementation.')
[void]$sb.AppendLine()
[void]$sb.AppendLine('::: warning Worksheet does not mean live Excel output')
[void]$sb.AppendLine('Excel is a held renderer in every run mode. Worksheet and Columns are internal manifest metadata')
[void]$sb.AppendLine('retained for compatibility and rebuild work; live outputs are React, Json, and JsonEvidence.')
[void]$sb.AppendLine(':::')
[void]$sb.AppendLine()
...
[void]$sb.AppendLine('produced nothing** — that file, not this page, tells you whether an empty collector means')
