...
$syntheticVerdicts = @{
    ...
    'azsc/subscription/securitypolicysweep'        = @{ Wired=$true; Reason='AB#7059 -- security.defenderAlerts[]/defenderAssessments[]/defenderSecureScores[] and governance policyComplianceStates now read the rest of the same Get-ScoutSubscriptionSecurityPolicySweep call (gated on -IncludePolicyCompliance); only DefenderPricing (duplicates security.defenderPlans) and SubscriptionDiagnosticSettings remain unwired from this sweep type' }
