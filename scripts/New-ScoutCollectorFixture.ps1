...
param(
    ...
    # Keep the committed estate for collectors that already have one; derive only the new ones.
    # See the block above the write for why adding a collector must not silently re-derive its
    # neighbours' fixtures.
...
# -PreserveExisting keeps the committed estate for collectors that already have one and only adds
# entries for collectors that do not (AB#6741).
#
# Why it is needed. The committed fixtures were generated by an EARLIER build of this script, and
# regenerating one wholesale today drops paths the old build derived and this one does not --
# `Security/DefenderAlerts` loses `properties.ConfidenceLevel`, so a column that used to compare a
# real value now compares 'N/A'. That is a silent WEAKENING of the proof, arriving as an unexplained
# golden diff on a collector nobody touched. Adding a collector to a category must not do that.
#
# The rewrite is still available (omit the switch), and re-deriving these estates from the current
# generator is worth doing deliberately -- with the golden diffs reviewed one at a time, which is the
# opposite of doing it as a side effect.
if ($PreserveExisting -and (Test-Path -LiteralPath $OutputPath -PathType Leaf)) {
    $Existing = Get-Content -LiteralPath $OutputPath -Raw | ConvertFrom-Json
    $Merged = [ordered]@{}
    $Kept = 0
    foreach ($Name in @($Collectors.Keys)) {
        $Prior = $Existing.collectors.PSObject.Properties[$Name]
        if ($null -ne $Prior) { $Merged[$Name] = $Prior.Value; $Kept++ }
        else { $Merged[$Name] = $Collectors[$Name] }
    }
    $Fixture.collectors = $Merged
    # The shared blocks are re-derived per collector, so a category gaining a collector legitimately
    # gains subscriptions/retirements entries. Keep the prior ones and append only what is new,
    # matched on the identity each block is keyed by.
    foreach ($Block in @('subscriptions', 'retirements', 'unsupported')) {
        $PriorRows = @($Existing.$Block)
        $PriorKeys = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
        foreach ($Row in $PriorRows) { [void]$PriorKeys.Add(($Row | ConvertTo-Json -Depth 8 -Compress)) }
        $Additions = @(@($Fixture.$Block) | Where-Object { -not $PriorKeys.Contains(($_ | ConvertTo-Json -Depth 8 -Compress)) })
        $Fixture.$Block = @($PriorRows) + @($Additions)
    }
    Write-Host "[New-ScoutCollectorFixture] Preserved $Kept existing collector estate(s) in $Category"
}
