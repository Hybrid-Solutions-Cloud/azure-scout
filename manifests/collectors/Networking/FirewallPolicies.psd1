...
# never had one. Networking coverage-gap close-out: `microsoft.network/firewallpolicies` is the
# Firewall Manager policy object, a distinct resource from any `AzureFirewalls` instance it
# attaches to -- a real gap, see docs/reference/service-coverage-gap.md.
#
@{
    ResourceTypes = @(
...
$ResUCount = 1
                ...
                $RuleGroupCount = @($data.ruleCollectionGroups).Count
...
'@
    ...
    Fields = @(
        ...
        @{
            ...
            Expression = '$data.threatIntelMode'
        }
        @{
            Name = 'Rule Collection Group Count'
            Expression = '[string]$RuleGroupCount'
        }
        @{
            Name = 'Resource U'
    ...
    Export = @{
        WorksheetName = 'Firewall Policies'
        TableNamePrefix = 'FirewallPoliciesTable_'
        Columns = @(
            'Subscription'
            'Resource Group'
            'Name'
        ...
        NumberFormat = '0'
        ConditionalText = @(
            'New-ConditionalText Off -Range J:J'
