...
Optional roles, matching the interactive [permissions](permissions.md) model:
...
The local run-folder isolation described in [Output](output.md) still applies inside the
...
- [Parameters](parameters.md) — every parameter, including `-Automation`, `-StorageAccount`, and `-StorageContainer`
- [Permissions](permissions.md) — the full role model
- [GitHub Actions](github-actions.md) — the CI-triggered alternative
- [Output Files & Formats](output.md) — what gets generated and where
