...
| **Markdown/Azure DevOps Wiki output with Mermaid diagrams** | Yes | No | **Rejected.** Scout's renderer contract is HTML/PDF/Excel/PowerPoint/Word/React/PowerBI — adding a Markdown/Wiki-specific renderer is unrelated to this Feature's AC (HTML or PDF). |
...
| **Multiple renderer targets from one scored object** (HTML, PDF, Excel, PowerPoint, Word, React SPA, Power BI) | Yes | No — fixed HTML/CSV/Markdown/JSON | Confirms the report belongs in `src/report/renderers/` as another `Export-*` dispatched from `Export-Report.ps1`, consuming the same `Findings`/`Collect` contract every other renderer already does, rather than a parallel one-off pipeline. |
